...
class Agent:
                    ...
                    for d in MyActions[3]:
                        ...
                        for i in range(4):
                            if yay:
                                ...
                                """
                                if(guy.actions[2] != 0): #If he CAN cover
                                    #Errata: Add in clause for multiple Knights
                                    lst = state.getDamagePotential(x)
                                    sigma = lst[0]+lst[2]+lst[4]+lst[6]
                                    fatality = False
                                    for person in state.Teams[x]:
                                        if person == guy: continue
                                        if (person.hp[0] <= sigma):
                                            if(Z[1] != 2):
                                                yay = False
                                    pass
                                """
                                ...
                                if(Z[1] == 5): #If heal1
                                    lst = state.getDamagePotential(x)
                                    sigma1 = lst[0]+lst[2]+lst[4]+lst[6]
                                    sigma2 = lst[1]+lst[3]+lst[5]+lst[7]
                                    #printf("WOOO!!!! %s    %s   %s\n",lst,sigma1,sigma2)
                                    person = state.Teams[Z[2][0]][Z[2][1]]
                                    if(Z[2][1] == shielder):
                                        if(person.hp[0] > sigma2 + 40):
                                            yay = False
                                    else:
                                        if(person.hp[0] > sigma1 + 40):
                                            yay = False
                                """
    ...
    def choose(self,game,x):
        ...
        TheirActions = [current_state.get_actions((x+1)%2,i) for i in range(4)]
        TheirActions = snag_choices(TheirActions,8192)
